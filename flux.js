...
(function($){
	flux.slider = function(elem, opts) {
		...
		this.element.find('img, a img').each(function(index, found_img){
			...
			var uid = $(found_img).data('uid');
			if(uid){
				$(imgClone).data('uid', uid);
				_this._uidImgMap[uid] = imgClone;
			}
	...
	flux.slider.prototype = {
		...
		finishedLoading: function() {
			...
			this.updateCaption();
			this.options.finishedLoading && this.options.finishedLoading.call(this);
		},
		setupImages: function() {
			var img1 = this.getImage(this.currentImageIndex),
				img2 = this.getImage(this.nextImageIndex),
				css1 = {
					'background-image': 'url("'+img1.src+'")',
					'z-index': 101,
					'cursor': 'auto'
				};
			console.log("img1.src", img1.xxxx)
			this.image1.attr("data-uid", $(img1).data('uid') || null);
			this.image2.attr("data-uid", $(img2).data('uid') || null);
...
(function($){
	...
	flux.transition.prototype = {
		...
		run: function() {
			...
			this.slider.image1.attr("data-uid", null).css({
